...
const TopFeatured = ({title}) => {
    const axiosPublic = useAxiosPublic();
    const { isLoading, refetch, data: topFeatured = [] } = useQuery({
        queryKey: ['topFeatured'],
        queryFn: async () => {
            const res = await axiosPublic.get(`/products/topFeatured`);
            return res.data;
        }
    });
    ...
    return (
        <div  className='bg-blue-50'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {topFeatured .slice(0, showAll ? topFeatured.length : displayLimit).map((item, index) =>
                    <TrandingCard title={' Featured Products'} key={index} item={item} />
                )}
            </div>

            {!showAll && topFeatured.length>6 && (
